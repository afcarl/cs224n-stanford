...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            target_words = []
            # Look at words left of window
            for j in range(-window_size,0):
                if i+j <0:
                    target_words.append(start)
                else:
                    target_words.append(sentence[i + j])
            #Add target word
            target_words.append(sentence[i])
            #Look at words right of window
            for k in range(0,window_size):
                if i + 1+ k < len(sentence):
                    target_words.append(sentence[i+1+k])
                else:
                    target_words.append(end)
            target_words = sum(target_words,[]) #flatten list to meet shape of desired output
            windowed_data.append((target_words,labels[i]))
		### END YOUR CODE
