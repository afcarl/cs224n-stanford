...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h = cell(x[:, time_step, :], h)  # call the cell we created in the last part. overwrite h with its new value
                o_dropout = tf.nn.dropout(o_t, dropout_rate)
                y_t = tf.matmul(o_dropout, U) + b2
                preds.append(y_t)
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        # print("Labels Shape", self.labels_placeholder)
        # print("Pred shape", preds.get_shape())
        loss_tot = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        loss = tf.boolean_mask(loss_tot, self.mask_placeholder)
