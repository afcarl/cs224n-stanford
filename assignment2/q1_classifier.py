...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        weight = tf.Variable(tf.zeros(shape=(self.config.n_features, self.config.n_classes), dtype=tf.float32))
        bias = tf.Variable(tf.zeros(shape=(self.config.n_classes), dtype=tf.float32))
        linear = tf.matmul(self.input_placeholder, weight) + bias
        pred = softmax(linear)
        ### END YOUR CODE
        return pred
