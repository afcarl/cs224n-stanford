...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        emb_weights = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(emb_weights, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        # b1 = xavier_initializer([1, self.config.hidden_size])
        # b2 = xavier_initializer([1, self.config.n_classes])
    ...
    def add_loss_op(self, pred):
        ...
        probs = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
        # probs += l2_regularizer * tf.nn.l2_loss(self.W)
